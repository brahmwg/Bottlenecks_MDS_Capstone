...
```{r}
getwd()
survdata <- read_csv("~/Library/Mobile\ Documents/com~apple~CloudDocs/Documents/Personal/jel000-notes/Capstone/Bottlenecks_MDS_Capstone/survival_analysis/preprocessed_data/survival_analysis_small.csv")
...
```{r}
hatch_no_origin <- select(hatchsimple_clean, -origin)
hatch_no_origin
```
...
write_bayes_cjs = function(file_path = NULL) {
  ...
  jags_model = function() {
    ...
    # PRIORS AND CONSTRAINTS
    for(j in 1:J) {
      phi[j] ~ dbeta(1, 1) # Overall prior for survival probabilities between arrays
      p[j] ~ dbeta(1, 1) # Overall prior for detection probabilities between arrays
    }
    ...
    for (i in 1:N) {
      b0[i] ~ dnorm(mu_b0, tau_b0) # Individual-level random effect
      b1[i] ~ dnorm(mu_b1, tau_b1) # Individual-level random effect

      logit_phi[i] <- b0[i] + b1[i] * y[i, 1]
      phi_temp[i] <- ilogit(logit_phi[i])
    }
    ...
    for (i in 1:N) {
      for (j in (f[i] + 1):J) {
        ...
        # survival process: 
        z[i,j] ~ dbern(phi_temp[i] * z[i,j-1]) # Fish i in period j is a Bernoulli trial
...
write_bayes_cjs(file_path = 'CJS_model_edited_hier.txt')
...
write_bayes_cjs(file_path = 'CJS_model_edited_2.txt')
```
...
write_bayes_cjs = function(file_path = NULL) {
  ...
  jags_model = function() {
    ...
    for(j in 2:J) {
      phi[j] ~ dbeta(x,1) # survival probability between arrays
      p[j] ~ dbeta(1,1)
    }
...
write_bayes_cjs = function(file_path = NULL) {
  ...
  jags_model = function() {
    ...
    # Priors for intercept and location-specific slopes
    b0 ~ dnorm(0, 1/100)
    
    for (j in 1:J) {
      b1[j] ~ dnorm(0, 1/100)  # location-specific coefficients
    }
...
jags_data = prep_jags_cjs(
  ...
  tag_meta = hatchtotal,
  drop_col_nm = c("duty_cycle", "origin"),
...
```{r}
cjs_post = run_jags_cjs(file_path = 'CJS_model_edited_hier.txt',
                        jags_data = jags_data,
                        n_chains = 4,
                        n_adapt = 1000,
                        n_burnin = 2500,
                        n_iter = 2500,
                        n_thin = 5,
                        params_to_save = c("phi", "p", "survship", "b0", "b1", "phi_temp", "mu_b0", "mu_b1"),
                        rng_seed = 4)
```
...
```{r}
logit_subset = param_summ[which(param_summ$param_grp == "phi"),]
logit_subset
```

```{r}
logit_subset <- logit_subset[sample(1:nrow(logit_subset),30, replace=FALSE),]
ggplot(logit_subset, aes(x=site_num, y=mean)) +         

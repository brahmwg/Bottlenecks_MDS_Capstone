...
2.  [Introduction](#introduction) <br> 2.1. [Motivation](#motivation)
3.  [Data Products](#data_product) <br> 3.1. [Survival
    Analysis](#survival_analysis) <br> 3.2. [Outmigration
    Model](#further-analysis) <br> 3.3. [Species Classification
    Model](#further-analysis) <br> 3.4. [PL/Python
...
<img src="img/survival_analysis_fig1.png"/> After data from all 5 stages
are extracted in this manner, the datasets then go into a
...
all 5 stages. ![Proprocessing pipeline](img/preprocessing_survival.png)

#### 3.1.2 Data Science Methods
...
#### 3.2.4 Data Science Method
With the preprocessed data, we used a standard scaler to scale all the features 
with a similar scale. Next, we employed an XGBoost model to predict salmon 
outmigration on specific dates. For feature selection, we used Lasso regression 
to identify the most significant variables. Then we used cross-validation for 
hyperparameter tuning. The model will train based on the user's input on 
predicting year (e.g. if the user wants to predict for 2022, the model will use 
everything prior to 2022 as the training set.) 

For prediction, we applied a function that will turn all negative predictions 
to 0, which means that there were no outmigration on that date- then the model 
will return a range of dates on the predicting year based on the lower and 
upper quartile user decide. 

#### 3.2.5 Conclusion and Future Recommendation
We hope to better inform biologists on the outmigration date with this model. 
The future improvement can be done with exploring different modeling approaches 
and including different potential predictors. 
...
![Parameters of the model](img/species_prediction_fig1.png) The model
architecture consists of the input layer with a ReLU activation function
...
features of one fish are identified and fed into the model. ![Deep
learning prediction](img/species_prediction_fig2.png) The model tested
with a 95% accuracy, where the accuracy is defined as the number of
...
validation sets can be seen here. ![Accuracy and loss
curves](img/species_prediction_fig3.png)
...
the predicted probability. ![Prediction
table](img/species_prediction_fig4.png)
...
![Non-deterministic features](img/species_prediction_fig6.png) These two
sets of features are merged to create a complete dataset that is
...
branch and the probabilistic branch. ![Voting
classifier](img/voting_classifier.png)
...
![img1](../mds_deliverables/pipeline/img/pipeline.png)
...
2. Process corresponds the processing script where, depending on the pipeline, different transformations of the data will be performed.

3. Staging table corresponds the table where all the transformed data will be stored, in order to be read again by the Predict script. Having a staging table is very standard in the industry because it helps separate the extraction and transformation from the loading part of the pipeline, ensuring an easier debugging process.

4. Predict corresponds the script that will be performed on the transformed data. This will run the models and get a prediction which will then be stored in the Results table.

5. Results table corresponds where all the results from the model prediction will be stored.

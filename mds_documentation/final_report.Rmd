...
The data used in this modeling is from three seperated source- the tagging history from the Pacific Salmon Foundation, the river flow and level is from the National Water Data Archive: HYDAT and the temperature data is from ECCC/MSC’s National Climate Archive. 
The data used in for this model retrieved from five queries, they are stored in the data folder as: 
- cowichan_historic.csv
- data_salmon2.csv
- flow_2023.csv
- level_2023.csv
- northcochiwan_daily_temp.csv
...
With the preprocessed data, we used a standard scaler to scale all the features with a similar scale, then we used a xgboost model to predict the salmons that will outmigrate on a given date with Lasso regression for feature selection and cross-validation for hyperparameter tuning. The model will train based on the user's input on predicting year (e.g. if the user wants to predict for 2022, the model will use everything prior to 2022 as the training set.) 
For prediction, we applied a function that will turn all negative predictions to 0, which means that there were no outmigration on that date- then the model will return a range of dates on the predicting year based on the lower and upper quartile user decide. 

#### 3.2.5 Conclusion and Future Recommendation
We hope to better inform biologists on the outmigration date with this model. The future improvement can be done with exploring different modeling approaches and including different potential predictors. 
